...
/// \brief A standard spherical camera projection model.
class SphericalCamera : public ProjectionBase
{
 public:
  ...
  inline SphericalCamera(int imageWidth, int imageHeight, float_t horizontalFov, float_t verticalFov);

  ~SphericalCamera() = default;

  /// \brief Write the intrinsics into a vector.
  /// \param[out] intrinsics The intrinsics as a vector.
  inline void getIntrinsics(VectorXf& intrinsics) const override;

  /// \brief Overwrite all intrinsics from a vector.
  /// \param[in] intrinsics The intrinsics as a vector.
  ...
  /// \brief Project a 3D point in homogenous coordinates to 2D image coordinates.
  /// \param[in]  point      The 3D point in homogenous Euclidean coordinates.
  /// \param[out] imagePoint The 2D image coordinates.
  /// \return The projection status.
  ...
  /// \brief Project a batch of 3D points in homogenous coordinates to 2D image coordinates.
  ...
  /// \brief Return the string `"SphericalCamera"`.
 ...
 protected:
  ...
  /// * `intrinsics_[0]`: Horizontal FoV in radians.
  /// * `intrinsics_[1]`: Vertical FoV in radians.
